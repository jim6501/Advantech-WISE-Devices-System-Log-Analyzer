// Ported 1:1 from ../../../parsers/wifi_parser.js — do not change the parsing logic, only types/imports.
import type { ParseStrategy, ParseStrategyResult, RawLogEntry } from '../../types';
import { hexToAscii } from './utils';

interface WifiEventResult {
  text: string;
  info: string | null;
}

export const wifiParser: ParseStrategy = {
  parse(pe: number, record: string, index: number, allLogs: RawLogEntry[]): ParseStrategyResult {
    ...
    switch (pe) {
      ...
      case 15: {
        ...
        if (index > 0 && allLogs && allLogs[index - 1]) {
          ...
          if (prevLog.PE === 15 && parseInt((prevLog.Record ?? '').substring(0, 2), 16) === 0x05) {
            description = 'RF Event';
            const ssid = hexToAscii(record);
            details = `Disconnect Info (SSID) SSID: "${ssid}"`;
            return { description, details };
          }
        }
        const res = parseWifiEvent(record, index, allLogs);
        details = res.text;
        if (res.info) {
      ...
      case 13:
        ...
        details = parseConfigError(record);
        break;
      case 6: // SNTP
        description = 'SNTP Status';
        {
          const val = parseInt(record, 16);
          const sntpMap: Record<number, string> = { 0: 'No error', 1: 'DNS error', 2: 'No socket', 3: 'No reply', 4: 'Socket fatal' };
      ...
      case 8:
        ...
        {
          const val = parseInt(record, 16);
          const memMap: Record<number, string> = { 1: 'IO full', 2: 'IO overwrite', 3: 'System overwrite' };
      ...
      case 16: // P2P
        ...
        {
          const val = parseInt(record, 16);
          const p2pMap: Record<number, string> = { 1: 'Access control error', 2: 'Password error', 3: 'No QOS ACK' };
...
function parseWifiEvent(record: string, _index: number, _allLogs: RawLogEntry[]): WifiEventResult {
  ...
  switch (byte3) {
    case 0x01:
      text = `WLAN disconnect. Reason: ${otherBytes}`;
      break;
    case 0x02:
      text = `Unexpected event. Code: ${otherBytes}`;
      break;
    case 0x03:
      text = `Unexpected socket. Code: ${otherBytes}`;
      break;
    case 0x04:
      text = `Tx socket failed. Socket:${record.substring(2, 4)}, Reason:${record.substring(4)}`;
      break;
    case 0x05: {
      // Disconnect info.
    ...
    case 0x07:
      text = `RSSI Histogram. Data: ${otherBytes}`;
      break;
    case 0x08:
      text = `Unexpected WLAN policy. Policy:${record.substring(2, 4)}`;
      break;
    case 0x09: {
    ...
    case 0x0a:
      text = `WLAN RF reset. Result: ${otherBytes}`;
      break;
    case 0x0b:
      text = `Push connection fail. Error: ${otherBytes}`;
      break;
    case 0x0c:
      text = `Upload connection fail. Error: ${otherBytes}`;
      break;
    case 0x0d:
      text = `MQTT connection fail. Error: ${otherBytes}`;
      break;
    case 0x0e: {
      ...
      if (errCode === '0bffad') {
        ...
        info =
          'The error code 0bffad is defined as DHCP Server related. It is recommended that the customer use the BSSID binding feature to lock the device to a specific AP MAC address. This is particularly useful in environments where multiple APs share the same SSID, as it prevents the WISE device from roaming to a different AP and causing DHCP IP assignment issues.';
      } else if (errCode === '05ff93') {
        text = 'Device RF fatal error (05FF93)';
        info =
          'The error code 05ff93 is defined as a wrong password issue. The primary recommendation is to first verify the stability of the signal strength to prevent the device from being repeatedly disconnected by the AP, which would cause instability between the WISE device and the AP requiring RF Reset actions. This will also help avoid the occasional occurrence of the 05ff93 error.';
      } else if (errCode === '05ff9a') {
        text = 'Device RF fatal error (05FF9A)';
        info =
          'The error code 05ff9a causes connection instability. Currently, the only solution is to use the FW workaround mechanism. This mechanism primarily involves the FW automatically performing an RF Reset when the device experiences a disconnection (as shown in Flowchart 3-1). If the device is unable to re-establish a connection with the AP, a Reboot Interval can be configured (as shown in Flowcharts 3-2 & 4). When the system enters a System Reboot, it serves as the final software defense line. (Generally, regardless of whether the reboot is triggered manually by power cycling or by the system, the WISE device will have a more stable connection with the AP after the reboot.)';
      } else {
    ...
    case 0x0f:
      text = `Device RF abort error. (${record.substring(4)})`;
      break;
    case 0x10:
      ...
      break;
    case 0x11:
      text = `Net config error`;
      break;
    case 0x12:
      text = `Connection list full`;
      break;
    case 0x13:
      text = `Reboot interval timeout`;
      info =
        'Refer to step 3-3 of the Disconnection Flowchart: WISE FW instructs the MCU to perform an RF Reset at intervals of 15s, 30s, ... < N min. (Simultaneously, the system determines if the RF Reset Time has reached the manually configured N min limit.)';
      break;
    case 0x14: {
      const sType = parseInt(record.substring(2, 4), 16);
      const sTypeMap: Record<number, string> = { 1: 'Modbus', 2: 'Cloud', 3: 'SNTP', 4: 'UDPCFG', 5: 'P2P', 6: 'WebServer' };
    ...
    case 0x16: {
      ...
      const wModeMap: Record<number, string> = { 1: 'Disassociate', 2: 'Ping' };
      const wActMap: Record<number, string> = { 0: 'Reset', 1: 'Reboot', 2: 'Re-associate' };
      text = `RF WDT. Mode:${wModeMap[wMode] || wMode}, Action:${wActMap[wAction] || wAction}`;
      break;
    }
    case 0x17:
      text = `RF callback event. Code:${record.substring(2, 4)}`;
      break;
    case 0x18: {
    ...
    case 0x19:
      text = `RF module WiFi event. ID:${record.substring(2, 6)}`;
      break;
    default:
...
function parseConfigError(record: string): string {
