// Ported 1:1 from ../../../parsers/nbiot_parser.js — do not change the parsing logic, only types/imports.
import type { ParseStrategy, ParseStrategyResult } from '../../types';

export const nbiotParser: ParseStrategy = {
  parse(pe: number, record: string): ParseStrategyResult {
    let description = '';
    let details = '';
    ...
    switch (pe) {
      case 1:
        ...
        details = parseCellularInfo(record);
        break;
      case 12:
        description = 'Battery Status';
        details = parseBattery(record);
        break;
      case 13:
      ...
      case 7:
        description = 'Power Action';
        {
          ...
          const actionMap: Record<number, string> = { 1: 'Power on', 2: 'System restart', 3: 'Power off' };
...
function parseCellularInfo(record: string): string {
...
function parseBattery(record: string): string {
  // Table: No. 0-5
  const code = parseInt(record, 16);
  const map: Record<number, string> = {
    0: 'RTC battery low',
